...
import { TaskComments } from "@/components/crm/task-comments";
  ...
  return (
    <ul className="flex flex-col gap-2">
      {tasks.map((task) => {
        ...
        return (
          <li key={task.id} className="flex flex-col gap-2 rounded-md border border-border bg-surface p-3">
            <div className="flex items-center gap-3">
              ...
              >
                {expanded ? t("comment.hide") : t("comment.show")}
              </button>
            </div>
            {expanded ? <TaskComments taskId={task.id} isProjectTask={task.isProjectTask ?? false} /> : null}
          </li>
        );
      })}
    </ul>
