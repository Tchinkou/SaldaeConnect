import "server-only";
import { getEmailProvider } from "@/server/core/email/provider";

const COPY = {
  fr: {
    subject: (number: string) => `Nouvelle demande ${number}`,
    heading: "Nouvelle demande reçue",
    body: (number: string, serviceName: string, contactName: string) =>
      `${contactName} a envoyé une demande pour « ${serviceName} », enregistrée sous le numéro ${number}.`,
    cta: "Voir dans le CRM",
  },
  en: {
    subject: (number: string) => `New request ${number}`,
    heading: "New request received",
    body: (number: string, serviceName: string, contactName: string) =>
      `${contactName} sent a request for "${serviceName}", registered under number ${number}.`,
    cta: "View in the CRM",
  },
  ar: {
    subject: (number: string) => `طلب جديد ${number}`,
    heading: "تم استلام طلب جديد",
    body: (number: string, serviceName: string, contactName: string) =>
      `أرسل ${contactName} طلبًا لخدمة «${serviceName}»، مسجلاً تحت الرقم ${number}.`,
    cta: "عرض في إدارة العملاء",
  },
} as const;

export async function sendNewRequestNotificationEmail({
  to,
  requestNumber,
  serviceName,
  contactName,
  crmUrl,
  locale,
}: {
  to: string;
  requestNumber: string;
  serviceName: string;
  contactName: string;
  crmUrl: string;
  locale?: string;
}) {
  const copy = COPY[locale as keyof typeof COPY] ?? COPY.fr;
  const dir = locale === "ar" ? "rtl" : "ltr";
  const body = copy.body(requestNumber, serviceName, contactName);

  const html = `<!doctype html>
<html lang="${locale}" dir="${dir}">
  <body style="font-family: -apple-system, Segoe UI, sans-serif; background:#f4f5f7; padding:32px;">
    <div style="max-width:480px;margin:0 auto;background:#fff;border-radius:12px;padding:32px;">
      <h1 style="font-size:20px;color:#0B1220;">${copy.heading}</h1>
      <p style="color:#374151;line-height:1.6;">${body}</p>
      <p style="text-align:center;margin:32px 0;">
        <a href="${crmUrl}" style="background:#1D4ED8;color:#fff;padding:12px 24px;border-radius:8px;text-decoration:none;display:inline-block;">${copy.cta}</a>
      </p>
    </div>
  </body>
</html>`;

  await getEmailProvider().send({
    to,
    subject: copy.subject(requestNumber),
    html,
    text: `${copy.heading}\n\n${body}\n\n${crmUrl}`,
    tag: "opportunity.created_notification",
  });
}
